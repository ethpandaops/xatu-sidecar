...
// PayloadAttestationMessage represents a processed ePBS payload_attestation_message (PTC vote) event from gossipsub.
type PayloadAttestationMessage struct {
	duplicateCache *ttlcache.Cache[string, time.Time]
	event          *RawPayloadAttestationMessage
...
// RawPayloadAttestationMessage represents the raw payload attestation message data received from gossipsub.
type RawPayloadAttestationMessage struct {
...
// NewPayloadAttestationMessage creates a new PayloadAttestationMessage instance from raw event data.
func NewPayloadAttestationMessage(log logrus.FieldLogger, event *RawPayloadAttestationMessage, clockDrift time.Duration, wallclock *ethwallclock.EthereumBeaconChain, duplicateCache *ttlcache.Cache[string, time.Time], clientMeta *xatu.ClientMeta) *PayloadAttestationMessage {
	return &PayloadAttestationMessage{
		log:            log.WithField("event", "LIBP2P_TRACE_GOSSIPSUB_PAYLOAD_ATTESTATION_MESSAGE"),
...
// Decorate enriches the payload attestation message event with additional metadata and returns a decorated event.
func (e *PayloadAttestationMessage) Decorate(ctx context.Context) (*xatu.DecoratedEvent, error) {
	...
	decoratedEvent := &xatu.DecoratedEvent{
		...
		Data: &xatu.DecoratedEvent_Libp2PTraceGossipsubPayloadAttestationMessage{
			Libp2PTraceGossipsubPayloadAttestationMessage: &gossipsub.PayloadAttestationMessage{
				...
				PayloadPresent:    wrapperspb.Bool(e.event.PayloadPresent),
				BlobDataAvailable: wrapperspb.Bool(e.event.BlobDataAvailable),
			},
		},
	}
	...
	if err != nil {
		e.log.WithError(err).Error("Failed to get extra payload attestation message data")
	} else {
		decoratedEvent.Meta.Client.AdditionalData = &xatu.ClientMeta_Libp2PTraceGossipsubPayloadAttestationMessage{
...
// ShouldIgnore determines if the payload attestation message event should be ignored based on deduplication and age.
func (e *PayloadAttestationMessage) ShouldIgnore(_ context.Context) (bool, error) {
	if e.event == nil {
		return true, nil
	}
	...
	if retrieved {
		...
		}).Debug("Duplicate payload attestation message event received")
	...
	// ignore payload attestations that are more than 16 slots old
...
func (e *PayloadAttestationMessage) getAdditionalData(_ context.Context, timestamp time.Time) (*xatu.ClientMeta_AdditionalLibP2PTraceGossipSubPayloadAttestationMessageData, error) {
	wallclockSlot, wallclockEpoch, err := e.wallclock.FromTime(timestamp)
	if err != nil {
		return nil, fmt.Errorf("failed to get wallclock time: %w", err)
	}

	extra := &xatu.ClientMeta_AdditionalLibP2PTraceGossipSubPayloadAttestationMessageData{
